...
def list_img_file(directory):
    ...
    print(directory)
    if directory_exists("__pycache__"):
        print(old_list)
        old_list.remove("__pycache__")
    # print old_list
    new_list = []
...
def compress(choose, des_dir, src_dir, file_list):
    ...
    for infile in file_list:
        ...
        img.save(os.path.abspath(des_dir) + "/" + infile)
def compress_photo():
    '''调用压缩图片的函数
    '''
    src_dir, des_dir = ".", "../min-photos/"
...
def cut_photo():
    ...
    if directory_exists(src_dir):
        ...
        if file_list:
            ...
            for infile in file_list:
                img = Image.open(infile)
                Graphics(infile=infile, outfile=infile).cut_by_ratio()
        else:
            pass
    else:
